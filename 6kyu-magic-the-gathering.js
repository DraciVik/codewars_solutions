...
function battle(player1, player2) {
        ...
        let myLength = player2.length;
        if (playerObj.player1.length <= playerObj.player2.length) {
                myLength = playerObj.player1.length;
        }

        for (let i = 0; i < myLength; i += 1) {
                if (playerObj.player1[i][0] >= playerObj.player2[i][1]) {
                        playerObj.player2.splice(i, 1);
                }
                if (playerObj.player2[i][0] >= playerObj.player1[i][1]) {
                        playerObj.player1.splice(i, 1);
                }
        }
        return playerObj;
}
