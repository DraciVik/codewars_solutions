...
function automorphic(n) {
        const nSqrd = n ** 2;
        const nSqrdArray = String(nSqrd).split('');
        const lastDigitsArray = nSqrdArray.slice(nSqrdArray.length - String(n).length);
        const lastDigits = lastDigitsArray.join('');
        return lastDigits == n ? 'Automorphic' : 'Not!!';
}
