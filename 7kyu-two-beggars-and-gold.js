  ...
  while (originArr.length !== 0) {
    if (originArr[0] < originArr[originArr.length - 1]) {
      if (aArr.length > bArr.length) {
        bArr.push(originArr[originArr.length - 1]);
        originArr.splice(originArr.length - 1, 1);
      } else {
        aArr.push(originArr[originArr.length - 1]);
        originArr.splice(originArr.length - 1, 1);
      }
    } else {
      if (aArr.length > bArr.length) {
        bArr.push(originArr[0]);
        originArr.splice(0, 1);
      } else {
        aArr.push(originArr[0]);
        originArr.splice(0, 1);
      }
    }
  }
  return [
    aArr.reduce((cum, cur) => cum + cur, 0),
    bArr.reduce((cum, cur) => cum + cur, 0)
  ];
}
