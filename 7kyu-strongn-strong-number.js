...
function strong(n) {
        ...
        const numberArr = stringArr.map(value => Number(value));
        // Used recursion just to test it out. A for loop should be more performant
        ...
        // Factorializes every number and then sums it up
        const numberFactorialized = numberArr.map(value => factorialize(value)).reduce((acc, sum) => acc + sum, 0);

        return n === numberFactorialized ? 'STRONG!!!!' : 'Not Strong !!';
}
